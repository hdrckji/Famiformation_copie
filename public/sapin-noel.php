...
<head>
    ...
    <title>Formation - Le Sapin de Noël - Famiflora</title>
...
<body>
    ...
    <script>
        ...
        function startVideo() { player.playVideo(); document.getElementById('start-btn').disabled = true; document.getElementById('start-btn').innerText = "Vidéo en cours..."; }
        function onPlayerStateChange(event) {
            if (event.data == YT.PlayerState.ENDED) {
                fetch('valider_vue.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: 'nom_page=sapin_noel'
                }).then(() => { document.getElementById('successModal').style.display = 'flex'; });
            }
        }
    </script>
</body>
